...
In every software project, there is a moment when the team decides to install a Sonar server and to do something with warnings to increase code quality. _**But every team will fail.**_ This is a typical scenario:
...
Unfortunately, the default setting of IDEs is that warnings are turned off, suggesting that this is a good approach. But it is not. I recommend that the "default setting" should be to turn on all of them. And then the team can agree on a few ones that should be ignored.
...
These static code checkers are also available as plugins for our IDEs. So I simply suggest installing them locally at the developers and executing them before the commits at least on the new or modified code parts. So we will not commit "Sonar warnings" anymore!

## Always keep the warning count at zero
...
As I see, the software industry simply does not need code quality above a certain level. Customers and stakeholders do not want more _clean code_ or fewer warnings. Maybe these things seem to be unnecessary or too expensive.
